...
const rawTools: ToolData[] = [
  ...
  },
  {
    ...
    headline: 'Split PDF.',
    subtitle: 'Extract pages, break a PDF into separate files, or cut it into equal parts — all in your browser.',
    quickAnswer: 'To split a PDF, select it above and pick what should come out: one PDF of the pages you choose, a separate PDF for every page (delivered as a ZIP), the whole document divided into a number of equal parts, or parts that each stay under a size you set in MB. Page thumbnails let you click the pages you want, or type ranges like 1-5, 8. Splitting runs entirely client-side in your browser, so confidential contracts and reports never leave your device, and it works offline once the page has loaded.',
    icon: '🥞',
    category: 'PDF Tools',
    categorySlug: 'pdf-tools',
    faqs: [
  ...
  },
  {
    ...
    quickAnswer: 'To compress a PowerPoint file, drop your .pptx above and it is reduced immediately — typically by 40-80% for photo-heavy decks, because pasted pictures are stored at full camera resolution and displayed in a box a few inches wide. To hit a specific limit, use "Or fit a size" and enter it: the tool searches for the gentlest setting that still comes in under 10MB, 5MB, 2MB or whatever you need, and says so plainly if the number is not reachable. It reduces PowerPoint file size without losing quality on the slides themselves — text, layouts, speaker notes, charts, transitions and animations are untouched, and only oversized pictures are redrawn. Images already the right size, flat graphics that would grow as JPEGs, and anything with transparency are left alone and listed with the reason. Everything runs in your browser - the presentation never leaves your device.',
    icon: '🗜️',
    category: 'Document Tools',
    categorySlug: 'document-tools',
    faqs: [
      ...
      { question: 'How do I compress a PowerPoint to under 10MB for email?', answer: 'Use "Or fit a size", enter 10 MB, and the tool searches for the gentlest setting that still comes in under it — rather than compressing as hard as it can and giving away quality you did not need to lose. The same works for 25 MB (Gmail), 20 MB (Outlook), or the 5 MB and 2 MB limits many corporate servers and upload portals use. If your number is not physically reachable it says so and reports the smallest it could manage, instead of handing back a file that meets the limit and looks terrible.' },
      { question: 'How big is too big for a PowerPoint file?', answer: 'For emailing, the practical ceiling is whatever the recipient\'s mail server allows — commonly 25 MB on Gmail, 20 MB on Outlook and 10 MB on many corporate systems. For the file itself, PowerPoint copes with very large decks, but anything past roughly 100 MB becomes slow to open, slow to save and risky to present from. If a deck is that large and it is not full of video, it is almost certainly full of pictures stored at camera resolution, which is exactly what this tool reduces.' },
      { question: 'Can I reduce PowerPoint file size without losing quality?', answer: 'On the slides themselves, yes — text, fonts, layouts, charts, tables, transitions and animations are never re-encoded, so they are bit-for-bit what they were. For the pictures, "without losing quality" is not quite the right frame: the tool removes resolution that was never displayed. An image shown three inches wide does not need 4,000 pixels, and discarding the excess changes nothing you could see at that size. Pictures already displayed at their natural resolution are left byte-for-byte identical rather than re-encoded.' },
  ...
  },
  {
    ...
    quickAnswer: 'To compress a Word document, drop your .docx above and it is reduced immediately. A .docx is a ZIP archive, and when one is unexpectedly large it is nearly always the pictures: a screenshot from a high-resolution monitor can be 3,840 pixels wide and several megabytes, shown six inches across on the page. To reduce a Word document to a specific size, use "Or fit a size" and enter your limit — 2MB, 1MB, 200KB — and the tool finds the gentlest setting that still fits, or tells you honestly that it cannot. Text, styles, headings, tables, footnotes, comments and tracked changes are untouched, so the file is still fully editable. Everything runs in your browser - the document never leaves your device.',
    icon: '🗜️',
    category: 'Document Tools',
    categorySlug: 'document-tools',
    faqs: [
      ...
      { question: 'How do I reduce a Word document to 2MB or under 1MB?', answer: 'Use "Or fit a size", enter your limit, and the tool finds the gentlest setting that still fits underneath it. Portals and application forms commonly cap uploads at 2 MB, 1 MB or even 200 KB, and those are reachable for most documents whose size is in pictures. Where a number genuinely is not reachable — a document that is mostly embedded objects, or one with very little image weight to reclaim — it says so and reports the smallest it achieved, rather than returning something unreadable that meets the number.' },
      { question: 'Why is my Word file size so large?', answer: 'Nearly always pasted pictures, and usually screenshots. Word stores the image you inserted at its original resolution regardless of how small you drag it on the page, so a full-screen capture from a high-resolution monitor can add several megabytes while being displayed six inches across. Photos behave the same way. Other causes worth knowing about are embedded objects (a whole spreadsheet pasted in), retained revision history from long editing sessions, and large embedded font subsets — none of which image compression can reach.' },
  ...
  },
  {
    ...
    faqs: [
      ...
      { question: 'Why does my photo show no metadata at all?', answer: 'Most likely something already stripped it. Any service that re-compresses your photo for display drops the metadata as a side effect, which is why pictures saved back from the major social platforms are usually clean. Screenshots and images exported by many editors carry nothing either. It is worth checking rather than assuming, since the behaviour depends on the app and on how the photo was sent.' },
  ...
  },
  {
    ...
    faqs: [
      ...
      { question: 'Do I still need this if I post to social media?', answer: 'Often not for a normal feed post: the major platforms re-compress what you upload, and that drops the metadata along the way. The exposure is in every route that moves the file as a file — email attachments, marketplace listings, forum and dating-site uploads, cloud-drive links, and messaging apps sending "as a document" or "original quality", which deliberately skip the re-compression that would have cleaned it. Sharing modes differ between apps and change between versions, so check a photo rather than trusting a rule of thumb.' }
    ],
