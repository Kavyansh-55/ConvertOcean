...
<div class="content-card">
  ...
  <p>Every developer eventually gets the request: the data lives in an API, and a stakeholder wants it as a spreadsheet. This tool answers it without writing an export feature — paste or drop in the JSON array, download the .xlsx, done. Object keys become the header row, values land in typed cells (numbers as numbers, so sums and filters work immediately), and nested objects flatten into dot-notation columns the same way our <a href="/json-to-csv/">JSON to CSV</a> converter handles them.</p>
...
<div class="content-card">
  ...
  <p>Once you can see what is there, <a href="/exif-remover/">EXIF Remover</a> takes it out without re-compressing the image, and the <a href="/guides/what-your-photos-reveal/">guide to what photos reveal</a> explains field by field what each tag gives away and where metadata survives sharing. iPhone photos need converting first with <a href="/heic-to-jpg/">HEIC to JPG</a>, since Apple's format stores metadata in a different container. If the picture is also headed for a form with size limits, the <a href="/image-resizer/">Image Resizer</a> handles dimensions and file size, and the <a href="/privacy-first/">privacy-first approach</a> explains why all of these run on your device rather than ours.</p>
</div>
    `,
    faqs: []
  },
...
<div class="content-card">
  ...
  <p>It is worth being accurate about the risk instead of overselling it. Any service that re-compresses your photo for display discards the metadata as a side effect of re-encoding, which is why a normal feed post on a major platform is usually not where the exposure lies. It lies in every route where the file travels as a file, untouched:</p>
  <ul>
  ...
  <p>Sharing modes differ between apps and change between versions, so treat any list of "safe" platforms as a starting point rather than a guarantee — check a photo instead of trusting a rule of thumb. The <a href="/exif-viewer/">EXIF Viewer</a> shows what is actually there before you decide, and an image that has already been through a platform is often clean, which the tool will say rather than inventing work. The <a href="/guides/what-your-photos-reveal/">photo metadata guide</a> covers what each field gives away. iPhone HEIC files need <a href="/heic-to-jpg/">HEIC to JPG</a> first. For photos that also need to meet dimension or file-size rules, the <a href="/image-resizer/">Image Resizer</a> covers that step, and <a href="/png-to-jpg/">PNG to JPG</a> handles format changes. Everything here runs on your own machine, which is the only arrangement that makes sense for a tool whose entire job is to stop a file telling strangers where you live.</p>
</div>
    `,
    faqs: []
  }
